import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import axios from "axios";
import "./ReliefCamps.css";

function ReliefCamps() {
  const [camps, setCamps] = useState([]);

  const navigate = useNavigate();

  useEffect(() => {
    const token = localStorage.getItem("token");

    if (!token) {
      navigate("/");
      return;
    }

    axios
      .get("https://disaster-relief-management-system-bcio.onrender.com/relief-camps/")
      .then((res) => setCamps(res.data))
      .catch((err) => console.log(err));
  }, [navigate]);

  const handleLogout = () => {
    localStorage.removeItem("token");
    navigate("/");
  };

  const handleDelete = (id) => {
    if (!window.confirm("Delete this relief camp?")) return;
    axios
      .delete(`https://disaster-relief-management-system-bcio.onrender.com/relief-camps/${id}`)
      .then(() => {
        setCamps((prev) => prev.filter((c) => c.id !== id));
      })
      .catch((err) => console.log(err));
  };

  const totalCapacity = camps.reduce((sum, c) => sum + (c.capacity || 0), 0);

  return (
    <div className="dash">
      <header className="dash-nav">
        <div className="dash-logo">
          <span className="logo-dot" />
          Relief<span className="logo-accent">Ops</span>
        </div>

        <div>
          <button
            className="add-disaster-btn"
            onClick={() => navigate("/add-relief-camp")}
          >
            + Add Relief Camp
          </button>

          <button className="dash-logout" onClick={handleLogout}>
            Logout ↗
          </button>
        </div>
      </header>

      <section className="hero">
        <h1>Relief Camps Overview</h1>
        <p>All registered relief camps and their capacity</p>
      </section>

      <section className="metrics">
        <div className="metric-card glow-total">
          <div className="metric-icon">🏕️</div>
          <div>
            <div className="metric-value">{camps.length}</div>
            <div className="metric-label">Total Camps</div>
          </div>
        </div>

        <div className="metric-card glow-active">
          <div className="metric-icon">👥</div>
          <div>
            <div className="metric-value">{totalCapacity}</div>
            <div className="metric-label">Total Capacity</div>
          </div>
        </div>
      </section>

      <section className="timeline">
        {camps.map((c, i) => (
          <div
            className="timeline-item"
            key={c.id}
            style={{ animationDelay: `${i * 0.06}s` }}
          >
            <div className="timeline-dot dot-active" />

            <div className="timeline-card">
              <div className="tl-top">
                <span className="tl-icon">🏕️</span>
                <span className="tl-title">{c.name}</span>
              </div>

              <div className="tl-location">📍 {c.location}</div>
              <div className="tl-location">👥 Capacity: {c.capacity}</div>

              <div className="action-buttons">
                <button
                  className="edit-btn"
                  onClick={() => navigate(`/edit-relief-camp/${c.id}`)}
                >
                  Edit
                </button>

                <button
                  className="delete-btn"
                  onClick={() => handleDelete(c.id)}
                >
                  Delete
                </button>
              </div>
            </div>
          </div>
        ))}

        {camps.length === 0 && (
          <div className="empty">Nothing to show here right now.</div>
        )}
      </section>
    </div>
  );
}

export default ReliefCamps;