  ...
  return (
    <div className="dash">
      ...
      <section className="timeline">
        {filtered.map((d, i) => (
          <div className="timeline-item" key={d.id} style={{ animationDelay: `${i * 0.06}s` }}>
            <div className={`timeline-dot ${d.status === "active" ? "dot-active" : "dot-resolved"}`} />
            <div className="timeline-card">
              <div className="tl-top">
                <span className="tl-icon">{getIcon(d.title)}</span>
                <span className="tl-title">{d.title}</span>
                <span className={`tl-badge ${d.status === "active" ? "b-active" : "b-resolved"}`}>
                  {d.status}
                </span>
              </div>
